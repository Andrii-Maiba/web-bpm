...
import {Container, Form, Input, Button, Header, Message, Dimmer, Loader, Grid} from 'semantic-ui-react';
...
class CreateProcessPageContainer extends Component {
    state = {isCreatingValidationError: false}
    creatingFormFields

    componentDidMount() {
        this.props.getStartEventXml(this.props.match.params.key);
    }

    shouldComponentUpdate(nextProps) {
        if (nextProps.isCreated) {
            this.setState({isCreatingValidationError: false});
            // console.log("push to locationPath", this.props.locationPath);
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        if (prevProps.xmlStartEventData !== this.props.xmlStartEventData && this.props.xmlStartEventData !== null) {
            this.creatingFormFields = Array.from(Array.from(this.props.xmlStartEventData.childNodes).find(el => el.nodeName === "bpmn:startEvent").childNodes)
                .find(el => el.nodeName === "bpmn:extensionElements").childNodes[1].childNodes;
    ...
    handleSubmit = event => {
        this.props.createProcess(this.state.data, this.props.match.params.key);
        ...
        return (
            <Container>
                <div className='pageBody'>
                    {this.state.chosenProcessName ?
                        <Header textAlign='center' content={this.state.chosenProcessName}/> :
...
export default compose(injectIntl, withServices(), connect(mapStateToProps, mapDispatchToProps))(CreateProcessPageContainer);
